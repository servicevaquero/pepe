...
class test extends FunSuite with BeforeAndAfter {
  ...
  test("Viendo como responde el gateway") {
	...
	assert(thrownA.getMessage() == "Saldo insuficiente")//Es lo que yo quer�a
    ...
    gateWay.ingresarPago(tarjetaB, 50) // este no va a ser realizado porque ser�a saldo -20 --> java.lang.Exception: Saldo insuficiente
    ...
    assert(thrownB.getMessage() == "Saldo insuficiente")//Es lo que yo quer�a
    ...
    //se pone OFFLINE y despu�s ONLINE para corroborar que los valores no se alteran, ya que no quedan pendientes
    gateWay.setEstadoDeConexion(false)
